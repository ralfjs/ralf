...
func ApplyFixes(source []byte, fixes []Fix) ([]byte, []Conflict) {
	...
	cmpFix := func(a, b Fix) int {
		...
		return cmp.Compare(a.EndByte, b.EndByte)
	}
	sorted := fixes
	...
	// Validate ranges and detect overlaps in a single pass, computing the
	// exact output size so we allocate the result buffer once.
